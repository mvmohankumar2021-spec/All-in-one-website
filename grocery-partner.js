(async () => {
  ...
  let schema;
  try {
    const response = await fetch('/grocery-schema.json');
    if (!response.ok) throw new Error('Grocery form unavailable');
    schema = await response.json();
  } catch {
    const error = document.createElement('p');
    error.setAttribute('role', 'alert');
    error.textContent = 'The grocery form could not load. Please reload the page. If this continues, contact support.';
    form.querySelector('.profile-actions')?.before(error);
    return;
  }
  const panel = document.createElement('section');
  panel.className = 'grocery-onboarding'; panel.hidden = true;
  const style = document.createElement('style');
  style.textContent = '.grocery-onboarding{grid-column:1/-1;min-width:0;border:1px solid #d7e3ee;border-radius:14px;background:#f8fafc;padding:24px;color:#243c52}.grocery-onboarding[hidden]{display:none}.grocery-onboarding fieldset{min-width:0;border:0;border-top:1px solid #d7e3ee;padding:20px 0;margin:0}.grocery-onboarding legend{font-size:20px;font-weight:700;padding:8px 0}.grocery-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}.grocery-field{min-width:0}.grocery-field label{display:block;margin-bottom:8px}.grocery-field input,.grocery-field textarea{box-sizing:border-box;width:100%;min-width:0;min-height:46px}.grocery-field details{margin-top:6px}.grocery-field summary{cursor:pointer;color:#245577;font-size:13px}.grocery-field details p{background:#e8f1fa;padding:12px;border-radius:8px;font-size:14px}.grocery-agreement{display:flex!important;align-items:flex-start;gap:10px;margin:12px 0}.grocery-agreement input{width:20px!important;min-width:20px;height:20px}.grocery-onboarding a{color:#235b7d;text-decoration:underline}.grocery-status{white-space:pre-wrap}.grocery-status.error{color:#a42218;background:#fff0ed;padding:12px}.grocery-onboarding [aria-invalid=true]{outline:2px solid #b42318}@media(max-width:640px){.grocery-grid{grid-template-columns:1fr}.grocery-onboarding{padding:16px}}';
  document.head.append(style);
  const layout = document.createElement('link');
  layout.rel = 'stylesheet'; layout.href = '/grocery-layout.css'; document.head.append(layout);
  style.textContent += '.profile-actions>[hidden],.grocery-onboarding [hidden]{display:none!important}';
  const heading = document.createElement('h2'); heading.textContent = 'Grocery partner onboarding'; panel.append(heading);
  const intro = document.createElement('p'); intro.textContent = 'Business name and contact details are taken from your partner profile. Complete only the grocery services you selected. * Required for submission.'; panel.append(intro);
  const status = document.createElement('p'); status.className = 'grocery-status'; status.setAttribute('role', 'status'); panel.append(status);
  function field(def, host) {
    const [key, title, required, type, help] = def;
    const wrap = document.createElement('div'); wrap.className = 'grocery-field';
    const label = document.createElement('label'); label.htmlFor = `grocery-${key}`; label.textContent = title + (required ? ' *' : ' (optional)');
    ...
    if (type === 'textarea') wrap.classList.add('grocery-field-wide');
    input.setAttribute('aria-required', String(required));
    const header = document.createElement('div'); header.className = 'grocery-field-heading';
    const helpButton = document.createElement('button'); helpButton.type = 'button'; helpButton.className = 'grocery-help-button';
    ...
    const hint = document.createElement('p'); hint.className = 'grocery-help-tooltip'; hint.id = `grocery-help-${key}`; hint.textContent = help; hint.hidden = true; hint.setAttribute('role', 'tooltip');
  ...
  function closeHelp() {
    panel.querySelectorAll('.grocery-help-button').forEach(button => button.setAttribute('aria-expanded', 'false'));
    document.querySelectorAll('.grocery-help-tooltip').forEach(hint => hint.hidden = true);
  }
  panel.addEventListener('click', event => {
    const button = event.target.closest('.grocery-help-button');
    if (!button) return;
  ...
  panel.addEventListener('keydown', event => {
    ...
    const button = panel.querySelector('.grocery-help-button[aria-expanded="true"]');
    if (button) { closeHelp(); button.focus(); event.preventDefault(); }
  });
  document.addEventListener('click', event => { if (!event.target.closest('.grocery-help-button,.grocery-help-tooltip')) closeHelp(); });
  window.addEventListener('resize', closeHelp);
  document.addEventListener('scroll', event => { if (!event.target.closest?.('.grocery-help-tooltip')) closeHelp(); }, true);
  for (const section of schema.sections) {
    ...
    for (const definitions of [section.fields.filter(def => def[3] !== 'textarea'), section.fields.filter(def => def[3] === 'textarea')]) {
      for (let index = 0; index < definitions.length; index += 2) {
        const row = document.createElement('div'); row.className = 'grocery-field-pair';
  ...
  const documents = document.createElement('fieldset'); documents.innerHTML = '<legend>Supporting documents</legend><p>Upload current applicable registration/licence proof, store photographs, sample catalogue and relevant supplier or organic certification evidence. Existing profile documents remain available; do not upload duplicates.</p><input id="grocery-files" type="file" multiple accept="application/pdf,image/jpeg,image/png" aria-label="Select grocery supporting documents" hidden><button type="button" class="button button-outline">Choose documents</button><p class="grocery-file-status" role="status">PDF, JPEG or PNG; up to 4 files per upload, maximum 1.5 MB each.</p><p><a href="#certificateList">View uploaded profile documents</a></p>';
  panel.append(documents);
  documents.classList.add('grocery-documents');
  const uploadedList = document.createElement('ul'); uploadedList.dataset.uploadedDocumentList = ''; documents.append(uploadedList);
  const fileInput = documents.querySelector('input'), upload = documents.querySelector('button'), uploadStatus = documents.querySelector('.grocery-file-status');
  const documentHint = document.createElement('p');
  documentHint.id = 'grocery-help-documents'; documentHint.className = 'grocery-help-tooltip';
  ...
  const documentHelp = panel.querySelector('.grocery-help-button').cloneNode(true);
  ...
  agreements.className = 'grocery-agreements-section';
  schema.policies.forEach(([key,title,url]) => { const label = document.createElement('label'); label.className = 'grocery-agreement'; const input = document.createElement('input'); input.type = 'checkbox'; input.dataset.agreement = key; const link = document.createElement('a'); link.href = url; link.target = '_blank'; link.rel = 'noopener'; link.textContent = `I accept ${title} *`; label.append(input,link); agreements.append(label); });
  ...
  const save = document.createElement('button'); save.type = 'button'; save.className = 'button button-outline'; save.textContent = 'Save grocery details';
  const submit = document.createElement('button'); submit.type = 'button'; submit.className = 'button button-dark'; submit.textContent = 'Submit grocery partner application →';
  ...
  async function persist(submitted) {
    ...
    try {
      ...
      const result = await fetch('/api/vendor/grocery-onboarding',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const data = await result.json(); if (!result.ok) throw new Error(data.error); status.textContent=data.message;
  ...
  // Route keyboard submission to grocery without invoking unrelated profile handlers.
  ...
  sync();
  try { const result=await fetch('/api/vendor/grocery-onboarding'); if (!result.ok) return; const data=await result.json(); panel.querySelectorAll('[data-key]').forEach(f=>f.value=data.details[f.dataset.key]||''); agreements.querySelectorAll('input').forEach(f=>f.checked=data.details.agreements?.[f.dataset.agreement]===true); status.textContent=`Grocery status: ${data.status}. Reselect your grocery services above to continue a saved application.`; } catch { status.textContent='Could not load saved grocery details. Reload before editing.'; }
})();
