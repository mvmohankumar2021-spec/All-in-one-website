(async () => {
  ...
  let schema;
  try {
    const response = await fetch('/catering-schema.json');
    if (!response.ok) throw new Error('Catering form unavailable');
    schema = await response.json();
  } catch {
    const error = document.createElement('p');
    error.setAttribute('role', 'alert');
    error.textContent = 'The catering form could not load. Please reload the page. If this continues, contact support.';
    form.querySelector('.profile-actions')?.before(error);
    return;
  }
  const panel = document.createElement('section');
  panel.className = 'catering-onboarding'; panel.hidden = true;
  const style = document.createElement('style');
  style.textContent = '.catering-onboarding{grid-column:1/-1;min-width:0;border:1px solid #d7e3ee;border-radius:14px;background:#f8fafc;padding:24px;color:#243c52}.catering-onboarding[hidden]{display:none}.catering-onboarding fieldset{min-width:0;border:0;border-top:1px solid #d7e3ee;padding:20px 0;margin:0}.catering-onboarding legend{font-size:20px;font-weight:700;padding:8px 0}.catering-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}.catering-field{min-width:0}.catering-field label{display:block;margin-bottom:8px}.catering-field input,.catering-field textarea{box-sizing:border-box;width:100%;min-width:0;min-height:46px}.catering-field details{margin-top:6px}.catering-field summary{cursor:pointer;color:#245577;font-size:13px}.catering-field details p{background:#e8f1fa;padding:12px;border-radius:8px;font-size:14px}.catering-agreement{display:flex!important;align-items:flex-start;gap:10px;margin:12px 0}.catering-agreement input{width:20px!important;min-width:20px;height:20px}.catering-onboarding a{color:#235b7d;text-decoration:underline}.catering-status{white-space:pre-wrap}.catering-status.error{color:#a42218;background:#fff0ed;padding:12px}.catering-onboarding [aria-invalid=true]{outline:2px solid #b42318}@media(max-width:640px){.catering-grid{grid-template-columns:1fr}.catering-onboarding{padding:16px}}';
  document.head.append(style);
  const layout = document.createElement('link');
  layout.rel = 'stylesheet'; layout.href = '/catering-layout.css'; document.head.append(layout);
  style.textContent += '.profile-actions>[hidden],.catering-onboarding [hidden]{display:none!important}';
  const heading = document.createElement('h2'); heading.textContent = 'Catering partner onboarding'; panel.append(heading);
  const intro = document.createElement('p'); intro.textContent = 'Business name and contact details are taken from your partner profile. Complete only the catering services you selected. * Required for submission.'; panel.append(intro);
  const status = document.createElement('p'); status.className = 'catering-status'; status.setAttribute('role', 'status'); panel.append(status);
  function field(def, host) {
    const [key, title, required, type, help] = def;
    const wrap = document.createElement('div'); wrap.className = 'catering-field';
    const label = document.createElement('label'); label.htmlFor = `catering-${key}`; label.textContent = title + (required ? ' *' : ' (optional)');
    ...
    if (type === 'textarea') wrap.classList.add('catering-field-wide');
    input.setAttribute('aria-required', String(required));
    const header = document.createElement('div'); header.className = 'catering-field-heading';
    const helpButton = document.createElement('button'); helpButton.type = 'button'; helpButton.className = 'catering-help-button';
    ...
    const hint = document.createElement('p'); hint.className = 'catering-help-tooltip'; hint.id = `catering-help-${key}`; hint.textContent = help; hint.hidden = true; hint.setAttribute('role', 'tooltip');
  ...
  function closeHelp() {
    panel.querySelectorAll('.catering-help-button').forEach(button => button.setAttribute('aria-expanded', 'false'));
    document.querySelectorAll('.catering-help-tooltip').forEach(hint => hint.hidden = true);
  }
  panel.addEventListener('click', event => {
    const button = event.target.closest('.catering-help-button');
    if (!button) return;
  ...
  panel.addEventListener('keydown', event => {
    ...
    const button = panel.querySelector('.catering-help-button[aria-expanded="true"]');
    if (button) { closeHelp(); button.focus(); event.preventDefault(); }
  });
  document.addEventListener('click', event => { if (!event.target.closest('.catering-help-button,.catering-help-tooltip')) closeHelp(); });
  window.addEventListener('resize', closeHelp);
  document.addEventListener('scroll', event => { if (!event.target.closest?.('.catering-help-tooltip')) closeHelp(); }, true);
  for (const section of schema.sections) {
    ...
    const grid = document.createElement('div'); grid.className = 'catering-grid';
    ...
    for (const definitions of [section.fields.filter(def => def[3] !== 'textarea'), section.fields.filter(def => def[3] === 'textarea')]) {
      for (let index = 0; index < definitions.length; index += 2) {
        const row = document.createElement('div'); row.className = 'catering-field-pair';
  ...
  const documents = document.createElement('fieldset'); documents.innerHTML = '<legend>Supporting documents</legend><p>Upload current registration/licence proof, menu brochure, kitchen photographs and relevant event portfolio. Existing profile documents remain available; do not upload duplicates.</p><input id="catering-files" type="file" multiple accept="application/pdf,image/jpeg,image/png" aria-label="Select catering supporting documents" hidden><button type="button" class="button button-outline">Choose documents</button><p class="catering-file-status" role="status">PDF, JPEG or PNG; up to 4 files per upload, maximum 1.5 MB each.</p><p><a href="#certificateList">View uploaded profile documents</a></p>';
  panel.append(documents);
  documents.classList.add('catering-documents');
  const uploadedList = document.createElement('ul'); uploadedList.dataset.uploadedDocumentList = ''; documents.append(uploadedList);
  const fileInput = documents.querySelector('input'), upload = documents.querySelector('button'), uploadStatus = documents.querySelector('.catering-file-status');
  const documentHint = document.createElement('p');
  documentHint.id = 'catering-help-documents'; documentHint.className = 'catering-help-tooltip';
  ...
  const documentHelp = panel.querySelector('.catering-help-button').cloneNode(true);
  ...
  agreements.className = 'catering-agreements-section';
  schema.policies.forEach(([key,title,url]) => { const label = document.createElement('label'); label.className = 'catering-agreement'; const input = document.createElement('input'); input.type = 'checkbox'; input.dataset.agreement = key; const link = document.createElement('a'); link.href = url; link.target = '_blank'; link.rel = 'noopener'; link.textContent = `I accept ${title} *`; label.append(input,link); agreements.append(label); });
  ...
  const save = document.createElement('button'); save.type = 'button'; save.className = 'button button-outline'; save.textContent = 'Save catering details';
  const submit = document.createElement('button'); submit.type = 'button'; submit.className = 'button button-dark'; submit.textContent = 'Submit catering partner application →';
  ...
  async function persist(submitted) {
    ...
    try {
      ...
      const result = await fetch('/api/vendor/catering-onboarding',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const data = await result.json(); if (!result.ok) throw new Error(data.error); status.textContent=data.message;
  ...
  // Route keyboard submission to catering without invoking unrelated profile handlers.
  ...
  sync();
  try { const result=await fetch('/api/vendor/catering-onboarding'); if (!result.ok) return; const data=await result.json(); panel.querySelectorAll('[data-key]').forEach(f=>f.value=data.details[f.dataset.key]||''); agreements.querySelectorAll('input').forEach(f=>f.checked=data.details.agreements?.[f.dataset.agreement]===true); status.textContent=`Catering status: ${data.status}. Reselect your catering services above to continue a saved application.`; } catch { status.textContent='Could not load saved catering details. Reload before editing.'; }
})();
