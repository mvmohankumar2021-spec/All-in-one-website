...
class FreshFoodTests(BaseTests):
    def setUp(self):
        super().setUp()
        self.db.execute('CREATE TABLE fresh_food_partner_onboarding(account_id INTEGER PRIMARY KEY,details_json TEXT,application_status TEXT,updated_at INTEGER)')
        self.schema = json.loads((server.ROOT / 'fresh-food-schema.json').read_text())

    def save(self, payload):
        self.handler.read_json = lambda: payload
        self.handler.vendor_catering_onboarding(save=True, fresh_food=True)
        return self.result

    def valid(self):
        data = {f[0]: ('2' if f[3] == 'number' else 'Test details') for s in self.schema['sections'] for f in s['fields']}
        data.update(services=['Fruit Shop'], fruit='Departments', expiry='', submit=True, agreements={p[0]: True for p in self.schema['policies']})
        return data

    def test_draft_round_trip(self):
        self.assertEqual(self.save({'services':['Vegetable Shop'], 'vegetable':'Daily essentials'})[1], 200)
        self.handler.vendor_catering_onboarding(fresh_food=True)
        self.assertEqual(self.result[0]['details']['vegetable'], 'Daily essentials')

    def test_public_schema_route(self):
        self.handler.path = '/fresh-food-schema.json'
        self.handler.do_GET()
        self.assertEqual(self.result[0]['services'], self.schema['services'])

    def test_empty_submission_rejected(self):
        self.assertEqual(self.save({'services':['Fruit Shop'], 'submit':True})[1], 400)
    ...
    def test_capacity_and_service_rules(self):
        ...
        data = self.valid(); data['services'] = ['Meat Shop']
        self.assertIn('Meat', self.save(data)[0]['error'])
